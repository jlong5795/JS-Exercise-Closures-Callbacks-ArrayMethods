...
function processLastItem(stringList, callback) {
  const string = stringList[(stringList.length - 1)];
...
function processSum(numberList, callback) {
 ...
 }//reduce calls an anonymous function passing a blank accumulator and the array.
...
function getFullNames(runners) {
  ...
  runners.forEach(function(runners){
    newList.push(`${runners.last_name}, ${runners.first_name}`)
  });
  return newList;
}
...
function firstNamesAllCaps(runners) {
  return runners.map(function(currentValue){
    return currentValue.first_name.toUpperCase();
  });
}//Look at current value. Return the first name of that object in uppercase. Map returns an array for you.
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  ...
  const size = runners.filter(function(currentValue) {  return currentValue.shirt_size === tShirtSize;
...
function tallyUpDonations(runners) {
  let sum = runners.reduce((accumulator, runners) => accumulator + runners.donation, 0);
